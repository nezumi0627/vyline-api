...
//
// We need only the initiator side: mint the SRTP master keying
// material (TGK), encrypt an envelope key with the peer's stnpk via
// RSA-OAEP, then wrap TGK + parameters in KEMAC authenticated with
// HMAC-SHA1 derived from the envelope key.
...
async function rsaOaepEncrypt(spki: Uint8Array, plaintext: Uint8Array): Promise<Uint8Array> {
  ...
  const ct = await crypto.subtle.encrypt({ name: "RSA-OAEP" }, key, toArrayBuffer(plaintext));
