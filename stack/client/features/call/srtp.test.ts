...
Deno.test("SRTP preserves RTP extension headers while encrypting payload", async () => {
  ...
  assertEquals(Array.from(encrypted.subarray(0, 16)), Array.from(rtp.subarray(0, 16)));
  let differs = 0;
...
Deno.test("deriveSrtpContext: enforces 30-byte master keying", async () => {
  await assertRejects(() => deriveSrtpContext(new Uint8Array(29)), Error, "30 bytes");
});
...
Deno.test("srtpDecrypt rejects tampered packet", async () => {
  ...
  encrypted[14] ^= 0x01;
  await assertRejects(() => srtpDecrypt(ctx, encrypted), Error, "auth tag mismatch");
});
