...
Deno.test("derivePlanetMediaStreamKeying applies native stream label HMAC", () => {
  ...
  assertEquals(hex(out), "3ebc662d4d812b04d940362540d64ecf18bbcc2cb2253cf722676d1e2554");
...
Deno.test("derivePlanetMediaKeys produces opposite-direction SRTP keying material", () => {
  ...
  assertEquals(
    ...
    new Uint8Array([0x11, 0x22, 0x33, 0x44, 0, 0, 0, 0]),
  );
...
Deno.test("sha256 matches well-known empty-string vector", () => {
  ...
  assertEquals([...e].map((b) => b.toString(16).padStart(2, "0")).join(""), expected);
});
