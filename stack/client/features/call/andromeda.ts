...
import { buildAudioOffer, buildAudioOfferMikey, parseSdp, readCrypto, readKeyMgmt } from "./sdp.js";
import { buildMikeyPke, mikeyFromBase64, mikeyToBase64, parseMikey } from "./mikey.js";
import {
  buildRtp,
  deriveSrtpContext,
  parseRtp,
...
export class AndromedaTransport implements CallTransport {
  ...
  async register(): Promise<number> {
    ...
    const wwwAuth =
      challenge.headers["WWW-Authenticate"] ?? challenge.headers["Proxy-Authenticate"] ?? "";
  ...
  async invite(opts: {
    ...
    if (sdes) {
      ...
      if (km && km.proto === "mikey") {
        ...
        if (!opts.decryptKey) {
          throw new Error("INVITE: peer sent MIKEY-PKE answer but no decryptKey supplied");
        }
        remoteKey = await decryptMikeyKemac(mk, opts.decryptKey);
      }
    }
