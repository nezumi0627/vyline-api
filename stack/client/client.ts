...
import { createLiffClient, type LiffClient } from "./features/liff.js";
...
export class Client extends TypedEventEmitter<ClientEvents> {
  ...
  async getChat(chatMid: string): Promise<Chat> {
    ...
    // 退出・キック済みだと chats[0] が undefined になる
    if (!raw || typeof (raw as { chatMid?: string }).chatMid !== "string") {
      throw new Error(`getChat: chat not found (${chatMid})`);
    }
