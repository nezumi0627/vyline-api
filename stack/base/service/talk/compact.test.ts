...
Deno.test("packCompactPlainMessage writes type, MID type, seq, MID bytes, text, and suffix", () => {
  ...
  assertEquals(hex(packed), "02" + "00" + "00" + "00".repeat(16) + "05" + "efbbbf6869" + "00");
});
...
Deno.test("packCompactE2EEMessage writes E2EE version, binary chunks, and compact key ids", () => {
  ...
  const packed = packCompactE2EEMessage(1, mid, [salt, message, nonce, senderKeyId, receiverKeyId]);
  assertEquals(
    hex(packed),
    "05" +
      "02" +
...
Deno.test("decodeCompactMessageResponse throws compact error code", () => {
  ...
  const error = assertThrows(() => decodeCompactMessageResponse(Uint8Array.from(body)));
