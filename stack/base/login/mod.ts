...
import { type AuthTokenInput, parseAuthTokenInput } from "../request/auth_token.js";
...
export class Login {
  ...
  async login(options?: LoginOption) {
    if (!options) {
      ...
      await this.withQrCode({ v3: (options as { v3?: boolean }).v3 });
      ...
      await this.withPassword(options as Parameters<typeof this.withPassword>[0]);
    } else {
      await this.withQrCode();
    }
    await this.ready();
  ...
  public async checkQrCodeVerified(
    ...
    for (let i = 0; i < maxCount; i++) {
      try {
        ...
        if (/Timeout|timed out|status=408|status=410/i.test(msg) && i < maxCount - 1) {
          continue;
        }
        throw error;
      }
  ...
  public async qrCodeLoginV2(
    ...
    systemName: string = "vyline-device",
    autoLoginIsRequired: boolean = true,
  ): Promise<LooseType> {
    return await this.client.request.request(
      [
  ...
  public async respondE2EELoginRequest(opts: {
    ...
    const pkTuple: import("../thrift/mod.js").NestedArray = [];
