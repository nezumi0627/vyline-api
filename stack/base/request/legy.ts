...
import { resolveLineAccessToken } from "./auth_token.js";
...
export class LegyEncryptedTransport {
  ...
  async fetch(
    ...
    const payload =
      (leInt & 4) === 4 ? Buffer.concat([Buffer.from([leInt]), plaintext]) : plaintext;
    ...
    const status =
      decoded.headers["x-lc"] && decoded.headers["x-lc"] !== "200"
...
export function xxhash32(data: Uint8Array, seed = 0): number {
  ...
  while (offset <= len - 4) {
    h32 =
      Math.imul(rotl((h32 + Math.imul(readU32LE(data, offset), PRIME3)) >>> 0, 17), PRIME4) >>> 0;
    offset += 4;
  }
  while (offset < len) {
