import { type NestedArray, type ParsedThrift, type ProtocolKey, Protocols } from "../thrift/mod.js";
import { type BaseClient, InternalError } from "../core/mod.js";
import { shouldUseLegyEncryptedAccess } from "./auth_token.js";
import { LegyEncryptedTransport } from "./legy.js";
...
export class RequestClient {
  ...
  constructor(client: BaseClient) {
    ...
    // Desktop: DESKTOP:WINDOWS:… / 副端末: Line/{ver}（patchDesktopTransport で上書き可）
    if (deviceDetails.device === "DESKTOPWIN" || deviceDetails.device === "DESKTOPMAC") {
      this.userAgent = `DESKTOP:${deviceDetails.systemName}:${deviceDetails.systemVersion}(${deviceDetails.appVersion})`;
    } else {
  ...
  public getHeader(overrideMethod: string = "POST"): Record<string, string> {
    ...
    // Desktop Legy 実測: X-LAP=5（ANDROIDSECONDARY 共存時も害なし）
    if (this.client.device === "DESKTOPWIN" || this.client.device === "DESKTOPMAC") {
      header["x-lap"] = header["x-lap"] ?? "5";
    }
