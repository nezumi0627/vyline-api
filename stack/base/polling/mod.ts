  ...
  async initLegyPusher(cb?: () => void) {
    ...
    while (this.client.authToken) {
      ...
      try {
        ...
        failCount++;
        const delay = Math.min(BASE_DELAY_MS * 2 ** (failCount - 1), MAX_DELAY_MS);
        await sleep(delay);
        continue;
      }
      try {
        ...
        if (uptime < MAX_SURVIVE_MS) {
          // Connection died almost immediately — likely auth/protocol issue.
          failCount++;
        } else {
          failCount = 0;
        }
        const delay = Math.min(BASE_DELAY_MS * 2 ** failCount, MAX_DELAY_MS);
        await sleep(delay);
      } catch (error) {
