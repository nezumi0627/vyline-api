...
import { getH2EnabledFetchForNode } from "./h2_fetch.js";
...
export class Conn {
  ...
  async new(host: string, _port: number, path: string, headers: Record<string, string> = {}) {
    ...
    // used as-is.
    const fetchFn = (await getH2EnabledFetchForNode()) ?? this.client.fetch;
    ...
    await new Promise<void>((resolve) => {
      (async () => {
        const socket = await fetchFn(`https://${host}${path}`, {
          ...
          signal: abort.signal,
          // @ts-expect-error: undici の fetch 型に無いが Node 実行時に必要
          duplex: "half",
        });
        if (!socket.body) {
  ...
  onPacketReceived(dt: number, dd: Uint8Array) {
    ...
    if (dt === 1) {
      ...
      throw new Error(
        `PUSH not Implemented: type:${dt}, payloads:${bytesToHex(dd).slice(
          0,
          30,
        )}, len:${dd.length}`,
      );
